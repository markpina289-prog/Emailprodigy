...
var index_default = {
  async fetch(request, env) {
    ...
    console.log("=== Phishing Simulation Request ===");
    ...
    // Basic validation
    if (!email || !firstName) {
      console.error("Missing parameters - email and name are required");
    ...
    console.log("Valid email format. Sending phishing simulation...");
...
<body style="margin: 0; padding: 0; font-family: 'Roboto', Arial, sans-serif; background-color: #f5f5f5;">
    ...
    try {
      ...
      if (response.ok) {
        ...
        return new Response("Phishing simulation email sent to " + email, {
          status: 200,
          headers: { "Content-Type": "text/plain" }
        });
      } else {
